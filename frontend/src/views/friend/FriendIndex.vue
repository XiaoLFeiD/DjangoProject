...
async function loadMore(){
  ...
  let newAiFriends = []
  try{
    const res = await api.get('/api/ai_friend/get_list/', {
      params: {
        items_count: ai_friends.value.length,
      }
    })
    const data = res.data
    if(data.result === 'success'){
      newAiFriends = data.ai_friends
    }
    ...
    isLoading.value = false
     if (newAiFriends.length === 0) {
      hasFriends.value = false
    }else{
      ai_friends.value.push(...newAiFriends)
       await nextTick()
...
function removeFriend(friendId) {
  ai_friends.value = ai_friends.value.filter(f => f.id !== friendId)
}
